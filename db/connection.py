...
load_dotenv(dotenv_path="/root/llmchat_fininsight/.env")
...
for db_name in [USER_DB_NAME, PROJECT_DB_NAME, CHAT_DB_NAME]:
    with base_engine.connect() as conn:
        ...
        conn.commit()
print(f"DB connection string: mysql+pymysql://{DB_USER}:****@{DB_HOST}:{DB_PORT}/")

user_engine = create_engine(
    f"mysql+pymysql://{DB_USER}:{DB_PASSWORD}@{DB_HOST}:{DB_PORT}/{USER_DB_NAME}", echo=True, pool_recycle=1800, pool_pre_ping=True
)
project_engine = create_engine(
    f"mysql+pymysql://{DB_USER}:{DB_PASSWORD}@{DB_HOST}:{DB_PORT}/{PROJECT_DB_NAME}", echo=True, pool_recycle=1800, pool_pre_ping=True
)
chat_engine = create_engine(
    f"mysql+pymysql://{DB_USER}:{DB_PASSWORD}@{DB_HOST}:{DB_PORT}/{CHAT_DB_NAME}", echo=True, pool_recycle=1800, pool_pre_ping=True
)