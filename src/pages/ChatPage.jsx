...
useEffect(() => {
  ...
  // 디버깅용 출력
  console.log(':흰색_확인_표시: 토큰:', token, '| 이메일:', email);
  if (token && email) {
    ...
    console.log(":흰색_확인_표시: 로그인된 사용자:", name, email);
  } else {
...
//------------------------- 프로젝트 목록 불러오기 ------------------------------
  useEffect(() => {
    ...
    axios.get(`http://223.130.156.200:8000/project/list?email=${email}`)
      .then((res) => {
        console.log(":열린_파일_폴더: 프로젝트 목록 응답:", res.data);
        setProjects(Array.isArray(res.data) ? res.data : res.data.projects || []);
      })
      .catch((err) => console.error(":x: 프로젝트 목록 불러오기 실패:", err));
  }, [account.googleEmail]);
  // 채팅 스레드 (백 연동 예정)
  ...
  const handleSelectChat = (chatId) => {
    ...
    setSelectedProjectId(null); // :흰색_확인_표시: 추가: 프로젝트 선택 해제
  };
  // 프로젝트 선택
  const handleSelectProject = async (pid) => {
    setSelectedProjectId(pid);
    setSelectedChatId(null); // :흰색_확인_표시: 추가: 채팅 선택 해제
    console.log(":열린_파일_폴더: 선택된 프로젝트 ID:", pid);
    try {
      const res = await axios.get(`http://223.130.156.200:8000/project/chat/history?project_id=${pid}`);
      const chatsFromDB = res.data.chats || [];
      console.log(":말풍선: 프로젝트별 채팅 불러오기:", chatsFromDB);
      ...
      console.error(":x: 채팅 불러오기 실패:", err);
    }
  };
  // :흰색_확인_표시: 프로젝트 생성
  const createProject = async (data) => {
    try {
      ...
      console.log(":파일_폴더: 프로젝트 생성 결과:", res.data);
      // 목록 갱신
      ...
      console.error(":x: 프로젝트 생성 실패:", err);
      alert("프로젝트 생성 중 오류가 발생했습니다.");
    }
  };
// :흰색_확인_표시: 프로젝트 이름 변경 함수
const renameProject = async (projectId, patch) => {
  ...
  try {
    ...
    console.log(`:연필2: 프로젝트 이름 변경 완료: ${newName}`);
  } catch (err) {
    console.error(":x: 프로젝트 이름 변경 실패:", err);
  }
};


// :흰색_확인_표시: 프로젝트 삭제 함수 (ChatPage 쪽)
const deleteProject = async (projectId) => {
  ...
  try {
    ...
    console.log(`:휴지통: 프로젝트 ${projectId} 삭제 완료`);
  } catch (err) {
    console.error(":x: 프로젝트 삭제 실패:", err);
  }
};
  // 채팅 생성
  ...
  // :흰색_확인_표시: 메시지 전송 함수 (채팅 제목 자동 생성 추가)
  const sendMessage = useCallback(async (text, model, deepResearch) => {
    ...
    // :흰색_확인_표시: 첫 메시지인 경우 채팅 제목 자동 생성
    const currentChatMessages = messages[chatId] ?? [];
    if (currentChatMessages.length === 0) {
      // 비동기로 실행 (메시지 전송을 차단하지 않음)
      generateChatTitle(chatId, text);
    }
    ...
    try {
      ...
      console.error(":x: 요청 중 오류 발생:", error);
      const errorMsg = {
        id: `m-${uuidv4()}`,
        role: "assistant",
  ...
  }, [selectedChatId, selectedProjectId, messages, generateChatTitle]);
  // 계정 수정 저장
  const saveAccount = (patch) => setAccount((a) => ({ ...a, ...patch }));
  // :흰색_확인_표시: 디버깅: sendMessage 함수 확인
  console.log(":전구: ChatPage 렌더링 - sendMessage 타입:", typeof sendMessage);
  return (
    <div style={{ display: "flex", height: "100vh", overflow: "hidden" }}>
      <Sidebar
        collapsed={sidebarCollapsed}
        onToggleCollapse={() => setSidebarCollapse((v) => !v)}
        account={account}
        onSaveAccount={saveAccount}
        projects={projects || []}   // :흰색_확인_표시: undefined 방지 -------------------- 수정
        setProjects={setProjects}
        chats={chats || []}         // :흰색_확인_표시: undefined 방지
